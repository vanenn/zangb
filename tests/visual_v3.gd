...
func execute():
	...
	if game.run.difficulty!="easy": issues.append("Difficulty not applied")
	...
	await click("BuildCodex")
	await capture("17-build-codex")
	game.start_run("teacher",1198)
	game.run.wave=1
	game._begin_wave()
	if game.screen!="equipment" or game.run.equipment_choices.is_empty(): issues.append("Early equipment guarantee missing")
	var core_options=game.run.equipment_choices.duplicate()
	game._continue_run()
	if game.run.equipment_choices!=core_options or game.screen!="equipment": issues.append("Guaranteed reward changed on reload")
	await click("Equip_"+game.run.equipment_choices[0])
	if not game.run.first_core or game.screen!="camp": issues.append("Guaranteed equipment did not return to camp")
	game._begin_wave()
	game.run.health = 0
	...
	var file = FileAccess.open("res://tests/visual-v3-result.json",FileAccess.WRITE)
