...
func execute():
	...
	check(state.weapon("chain").jumps == 9,"level three and five add chain jumps")
	state.roster.clear()
	...
	# Version one fields and RNG are retained; immutable pre-upgrade backup survives writes.
	state.mode = "camp"
	shop.stock(state,storage.meta.unlocked)
	var legacy = state.snapshot()
	legacy.version = 1
	legacy.erase("map_state")
	storage._write("run.json",legacy)
	var restored = State.new(book)
	check(restored.restore(JSON.parse_string(JSON.stringify(legacy))) and restored.map_state.is_empty(),"legacy checkpoint migration with default scene state")
	restored.map_state = city.snapshot()
	check(storage.save_run(restored) and FileAccess.file_exists(storage.directory.path_join("run.pre-v2.json")),"immutable pre-v2 checkpoint backup")
	check(restored.snapshot().version == 2,"checkpoint writes version two")
	var file = FileAccess.open("res://tests/v2-result.json",FileAccess.WRITE)
	file.store_string(JSON.stringify({"checks":checks,"failures":failures},"\t"))
	file.close()
	print("V2 CHECKS ",checks," FAILURES ",failures)
	sim.queue_free()
	city.queue_free()
	await process_frame
